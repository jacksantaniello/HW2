...
function begin_play(){
	...
	var player1 = document.getElementById("player1_id"); // player 1 text entry
	var player2 = document.getElementById("player2_id"); // player 2 text entry
...
*/
function alertWinner(turnChecker) {
	if (turnChecker == 1){
		alert("Winner is: X")
	} else {
		alert("Winner is: O")
...
function checkWin() {
	var turnChecker = whose_move();
	if(board_state[0] == turnChecker && board_state[1] == turnChecker && board_state[2]== turnChecker) {
		alertWinner(turnChecker);
	} else if (board_state[0] == turnChecker && board_state[3] == turnChecker && board_state[6]== turnChecker) {
		alertWinner(turnChecker);
	} else if (board_state[0] == turnChecker && board_state[4] == turnChecker && board_state[8]== turnChecker) {
		alertWinner(turnChecker);
	} else if (board_state[1] == turnChecker && board_state[4] == turnChecker && board_state[7]== turnChecker) {
		alertWinner(turnChecker);
	} else if (board_state[2] == turnChecker && board_state[4] == turnChecker && board_state[6]== turnChecker) {
		alertWinner(turnChecker);
	} else if (board_state[2] == turnChecker && board_state[5] == turnChecker && board_state[8]== turnChecker) {
		alertWinner(turnChecker);
	} else if (board_state[3] == turnChecker && board_state[4] == turnChecker && board_state[5]== turnChecker) {
		alertWinner(turnChecker);
	} else if (board_state[6] == turnChecker && board_state[7] == turnChecker && board_state[8]== turnChecker) {
		alertWinner(turnChecker);
	}
}
...
function play() {
	...
	if(table_ids.includes(input) == false || board_state[table_ids.indexOf(input)] != -1) {
		alert("Not a valid move please enter move.")
		return;
	}
	if(whose_move() == 1){
		board_state[table_ids.indexOf(input)] = 1;
		document.getElementById(input).innerText = "X";
		document.getElementById("turn_info").innerText = "Turn for: O";
	} else {
		board_state[table_ids.indexOf(input)] = 0;
